...
Token-level compression methods---including LLMLingua~\citep{jiang2023llmlingua,jiang2024longllmlingua} and KV-cache eviction approaches such as H2O~\citep{zhang2023h2o} and SnapKV~\citep{li2024snapkv}---operate at the lexical level, treating a numerical constant and a discourse adjective equivalently. SLSC instead compresses over semantic units (atomic mathematical propositions) via a dedicated curator agent. We view the two approaches as orthogonal and restrict our comparison to state-maintenance baselines sharing the same multi-turn setup.
...
long-horizon reasoning by iteratively  summarizing chain-of-thought~\citep{wei2022chain} and re-prompt the model on the summary, achieving large context savings.
Conceptually these methods sit closest to SLSC, and two differences are central. First, both produce flat natural-language summaries in which numerical constraints stated in passing receive no privileged representation; SLSC instead enforces an atomic-proposition schema in which each constraint occupies an independently addressable slot. Second, InftyThink targets monolithic single-question reasoning with internally-induced summarization points and assumes a trained summarization checkpoint, whereas our setting is externally-decomposed multi-turn dialogue with explicit numerical dependencies and is fully training-free. A head-to-head
...
annotating correctness at each intermediate step; \citet{lightman2024lets} similarly verify reasoning step by step using human-annotated process rewards. These works target end-to-end proof construction or single-statement formalization, with verifier feedback in the loop. SLSC's Lean~4 augmentation
