...
We propose \emph{Sparse Logic State Compression} (SLSC), a twin-agent framework~\citep{wu2023autogen,yao2023react} where a reasoning agent $\mathcal{A}$ generates natural-language chains of thought~\citep{wei2022chain} while a curator agent $\mathcal{B}$ distills accumulated context into atomic mathematical propositions. The reasoning agent's context is replaced by this structured state, giving each constraint a persistent, addressable location. Figure~\ref{fig:comparison} contrasts the two representations.
