...
\subsection{Agent B Curation Algorithm}
\label{app:curation_alg}
...
\subsection{SLSC Multi-Turn Inference Algorithm}
\label{app:inference_alg}
...
\label{app:extraction_example}

The following shows a representative extraction on a divisibility problem:
...
\subsection{State Representation Example}
\label{app:state_example}

The following shows a typical $S_t$ after three curation steps, with optional Lean~4 stubs:
...
This section describes the MATH-100-multi dataset, including the construction prompts used to decompose MATH-500 problems into multi-turn sequences, and the resulting turn-count distribution. The decomposition pipeline uses Gemma~4 31B~\citep{gemmateam2026gemma4} as the backbone model.
...
We use MATH-500 (MIT License), Qwen3.5 (Apache 2.0), Gemma-3-27B~\citep{gemmateam2025gemma3} (Gemma Terms of Use), Gemma~4 31B~\citep{gemmateam2026gemma4} (Gemma Terms of Use), and vLLM~\citep{kwon2023efficient} (Apache 2.0), all in accordance with their respective licenses for non-commercial research purposes. Our released artifacts---the Math-100-multi dataset and SLSC implementation code---will be made available under the MIT License upon publication.

All models are served using vLLM~\citep{kwon2023efficient} with \texttt{enable\_thinking=True} to activate the extended reasoning mode of Qwen3.5 models. Inference uses greedy decoding: temperature~$= 0$, top-$p = 1.0$, and \texttt{max\_tokens}~$= 16{,}384$. Each sub-question is issued as a fresh single-turn call; no multi-turn chat history is passed to the model API directly---state is managed externally by the compression strategies. Agent~$\mathcal{B}$ (curator) calls use the same vLLM instance with \texttt{max\_tokens}~$= 512$. The LLM judge (Kimi~K2.6~\citep{moonshotai2026kimik26}) is called via its public API with \texttt{temperature}~$= 0$.

\paragraph{Compute and infrastructure.}
All experiments were conducted on a single NVIDIA RTX PRO 6000
Blackwell Workstation Edition (96GB GDDR7). A single
reasoner-condition run over the 100 evaluation problems takes
approximately 1 GPU-hour. Aggregating Agent $\mathcal{A}$
inference across 4 reasoner scales (Qwen3.5-0.8B/2B/4B/9B) and
4 compression conditions (No Compression, Summary, SLSC w/o Lean,
SLSC with Lean) yields approximately 16 GPU-hours; including
Agent $\mathcal{B}$ (Gemma3-27B) curation calls across the three
compression conditions adds roughly 10 GPU-hours, for a total of
approximately 25--30 GPU-hours of local inference. The LLM judge
(Kimi K2.6) is API-based and not included in this estimate.
